...
Notes:
  ...
  - Actual provisioning implementation in this version is libvirt + Ubuntu only.
...
if [[ "${OS_FAMILY}" != "ubuntu" ]]; then
  if [[ "${OS_FAMILY}" == "gentoo" ]]; then
    die "Provisioning interface parsed --os gentoo --init ${INIT_SYSTEM}, but Gentoo provisioning is not implemented yet in v1 (current implementation: libvirt + ubuntu)"
  fi
  die "Provisioning interface parsed --os ${OS_FAMILY}, but ${OS_FAMILY} provisioning is not implemented yet in v1 (current implementation: libvirt + ubuntu)"
fi
...
DEPLOYMENT_VM_NAME="${DEPLOYMENT_VM_NAME:-compose-traeffik-ubuntu}"
...
DEPLOYMENT_SSH_USER="${DEPLOYMENT_SSH_USER:-ubuntu}"
...
if [[ ("${ACTION}" == "apply" || "${ACTION}" == "plan") && "${SKIP_IMAGE_FETCH}" != "true" ]]; then
  mkdir -p "$(dirname "${DEPLOYMENT_UBUNTU_IMAGE_PATH}")"
  if [[ ! -f "${DEPLOYMENT_UBUNTU_IMAGE_PATH}" ]]; then
    log "Downloading Ubuntu cloud image to ${DEPLOYMENT_UBUNTU_IMAGE_PATH}"
    curl -fL "${DEPLOYMENT_UBUNTU_IMAGE_URL}" -o "${DEPLOYMENT_UBUNTU_IMAGE_PATH}"
  else
    log "Using existing Ubuntu image: ${DEPLOYMENT_UBUNTU_IMAGE_PATH}"
  fi
fi

if [[ "${ACTION}" == "apply" || "${ACTION}" == "plan" ]]; then
  [[ -f "${DEPLOYMENT_UBUNTU_IMAGE_PATH}" ]] || die "Ubuntu image file not found: ${DEPLOYMENT_UBUNTU_IMAGE_PATH}"
fi
...
TF_VARS=(
  ...
  "-var=ubuntu_image_path=${DEPLOYMENT_UBUNTU_IMAGE_PATH}"
...
case "${ACTION}" in
  plan)
    log "Planning ${OS_FAMILY} VM '${DEPLOYMENT_VM_NAME}' on ${TARGET}"
    terraform -chdir="${TF_DIR}" plan "${TF_VARS[@]}"
    ;;
  apply)
    log "Applying ${OS_FAMILY} VM '${DEPLOYMENT_VM_NAME}' on ${TARGET}"
    APPLY_ARGS=()
    ...
    ;;
  destroy)
    log "Destroying VM '${DEPLOYMENT_VM_NAME}' on ${TARGET}"
    DESTROY_ARGS=()
    if [[ "${AUTO_APPROVE}" == "true" ]]; then
  ...
  ssh)
    ...
    exec ssh -o StrictHostKeyChecking=accept-new "${ssh_user}@${host_ip}"
    ;;
esac
