...
mkdir -p "${REPO_ROOT}/services/n8n/rendered"
if [ ! -f "${REPO_ROOT}/services/n8n/rendered/stepca-root-ca.crt" ]; then
    : > "${REPO_ROOT}/services/n8n/rendered/stepca-root-ca.crt"
    log_info "Created n8n step-ca trust placeholder file."
fi
if [ ! -f "${REPO_ROOT}/services/n8n/rendered/n8n.env" ]; then
    cat > "${REPO_ROOT}/services/n8n/rendered/n8n.env" <<'EOF'
# Placeholder generated by scripts/env-generate.sh.
# Run `make n8n-bootstrap` before enabling the `n8n` profile.
N8N_RENDER_STATUS=placeholder
NODE_EXTRA_CA_CERTS=
EOF
    log_info "Created n8n rendered env placeholder."
fi
