# Documentation Index

Welcome to the documentation for the **Traefik Docker Compose Edge Stack**.

This index links to the core references and service guides under `docs/`.

## Start Here

- [Documentation Index (Detailed)](00-index.md)
- [Repo Facts (Source of Truth)](90-facts.md)
- [Documentation Quality Checklist](98-doc-qa.md)
- [Style Guide](99-style-guide.md)
- [Glossary](99-glossary.md)

## Service Guides

- [DNS Service (BIND)](06-howto/service-dns-bind.md)

## TLS Mode Guides

- [Mode A: Local Self-Signed CA + Certificates](05-tls/mode-a-selfsigned.md)
- [Mode B: Let's Encrypt via Certbot](05-tls/mode-b-letsencrypt-certbot.md)
- [Mode C: Smallstep `step-ca` as Internal ACME Server](05-tls/mode-c-stepca-acme.md)

## How-to Guides

- [How-to: Trust Step-CA on Ubuntu 24.04](06-howto/stepca-trust-ubuntu.md)
