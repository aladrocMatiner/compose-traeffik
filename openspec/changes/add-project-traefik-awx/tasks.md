...
- [ ] 1.1 Confirmar `traefik-awx` como proyecto soportado en el catálogo.
...
- [ ] 1.4 Validar artefactos del cambio con `openspec validate add-project-traefik-awx --strict`.

## 2. Project Definition

- [ ] 2.1 Crear estructura `deployment/projects/traefik-awx/`.
...
- [ ] 2.3 Registrar `traefik-awx` en el catálogo para `deployment-project-list`.

## 3. Deployment Behavior

- [ ] 3.1 Implementar tasks Ansible para sync de `compose-traeffik` en la VM objetivo.
- [ ] 3.2 Ejecutar compose para los servicios del proyecto AWX definidos en manifiesto.
...
- [ ] 3.5 Integrar autenticación de AWX con Keycloak según contrato del proyecto.
- [ ] 3.6 Garantizar idempotencia razonable del flujo.
- [ ] 3.7 Asegurar que AWX se publica detrás de Traefik y que la terminación TLS la gestiona Traefik según `tls_mode`.
...
- [ ] 4.3 Documentar ejecución de `project=traefik-awx` y override TLS.
...
- [ ] 5.1 Re-ejecutar `openspec validate add-project-traefik-awx --strict`.
