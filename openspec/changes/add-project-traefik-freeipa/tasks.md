...
- [ ] 1.1 Confirm `traefik-freeipa` as a valid project identifier in deployment catalog scope.
- [ ] 1.2 Confirm dependency and TLS baseline contract for `traefik-freeipa` (StepCA ACME default with explicit override support).
- [ ] 1.3 Confirm this change is deployment-only and explicitly excludes service runtime implementation.
- [ ] 1.4 Validate artifacts with `openspec validate add-project-traefik-freeipa --strict`.

## 2. Deployment-Side Project Definition

- [ ] 2.1 Create `deployment/projects/traefik-freeipa/` manifest scaffold.
- [ ] 2.2 Register `traefik-freeipa` in project catalog wiring for list/discovery commands.
...
- [ ] 3.1 Add deployment preflight check that detects missing FreeIPA service/profile implementation.
...
- [ ] 5.1 Re-run `openspec validate add-project-traefik-freeipa --strict`.
