...
- [x] 1.1 Review and approve qemu image-profile scope for `Fedora Cloud` (`vm-provisioning` only vs full bootstrap parity).
...
- [x] 3.1 Confirm `Fedora Cloud` image supports cloud-init with hostname + SSH key injection.
...
- [x] 4.2 Document known limitations and follow-up needs for Docker bootstrap parity on `Fedora Cloud`.
