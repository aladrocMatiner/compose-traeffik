...
- [ ] 1.2 Validate change artifacts with `openspec validate add-qemu-fedora-cloud-image-support --strict`.

## 2. QEMU Image Profile Definition

- [ ] 2.1 Add a `fedora-cloud` OS/image profile for `target=qemu` in the provisioning interface.
- [ ] 2.2 Define pinned image source/version metadata for `Fedora Cloud` (and checksum strategy if available).
...
- [ ] 4.1 Update `make help` / docs with the `fedora-cloud` profile selection example for qemu (when implemented).
