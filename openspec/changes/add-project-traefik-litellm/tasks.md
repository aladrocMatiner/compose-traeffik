...
- [ ] 1.1 Confirmar `traefik-litellm` como proyecto soportado en el catálogo.
...
- [ ] 1.4 Validar artefactos del cambio con `openspec validate add-project-traefik-litellm --strict`.

## 2. Project Definition

- [ ] 2.1 Crear estructura `deployment/projects/traefik-litellm/`.
...
- [ ] 2.3 Registrar `traefik-litellm` en el catálogo para `deployment-project-list`.

## 3. Deployment Behavior

- [ ] 3.1 Implementar tasks Ansible para sync de `compose-traeffik` en la VM objetivo.
- [ ] 3.2 Ejecutar compose para servicios de LiteLLM definidos en manifiesto.
- [ ] 3.3 Asegurar que LiteLLM se publica detrás de Traefik y que la terminación TLS la gestiona Traefik según `tls_mode`.
...
- [ ] 4.2 Añadir guardrails para prerequisitos TLS (StepCA ACME) y variables del runtime LLM.
- [ ] 4.3 Documentar ejecución de `project=traefik-litellm` y override TLS.
...
- [ ] 5.1 Re-ejecutar `openspec validate add-project-traefik-litellm --strict`.
