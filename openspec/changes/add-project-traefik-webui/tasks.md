...
- [x] 1.1 Confirmar `traefik-webui` como proyecto soportado en el catálogo.
...
- [x] 1.4 Validar artefactos del cambio con `openspec validate add-project-traefik-webui --strict`.

## 2. Project Definition

- [x] 2.1 Crear estructura `deployment/projects/traefik-webui/`.
...
- [x] 2.3 Registrar `traefik-webui` en el catálogo para `deployment-project-list`.

## 3. Deployment Behavior

- [x] 3.1 Implementar tasks Ansible para sync de `compose-traeffik` en la VM objetivo.
- [x] 3.2 Ejecutar compose para servicios de WebUI definidos en manifiesto.
- [x] 3.3 Asegurar que WebUI se publica detrás de Traefik y que la terminación TLS la gestiona Traefik según `tls_mode`.
...
- [x] 4.2 Añadir guardrails para prerequisitos TLS (StepCA ACME) y variables del runtime WebUI.
- [x] 4.3 Documentar ejecución de `project=traefik-webui` y override TLS.
- [x] 4.4 Añadir tests de wiring del catálogo y contrato proxy/TLS.
- [x] 4.5 Evitar overrides ad-hoc de servicios fuera del manifiesto.

## 5. Validation and Handoff

- [x] 5.1 Re-ejecutar `openspec validate add-project-traefik-webui --strict`.
