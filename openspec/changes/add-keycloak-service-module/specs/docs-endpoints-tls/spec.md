...
#### Scenario: Keycloak endpoint documented
- **WHEN** the Keycloak module is introduced
- **THEN** the root README documents the Keycloak endpoint, profile enablement, and Traefik/TLS access expectations
- **AND** it notes any hostname/hosts mapping steps required for local access
