...
### Requirement: Keycloak profile safety validation
The system SHALL validate Keycloak profile configuration before startup, including hostname label format, required non-placeholder secrets, and reverse-proxy-safe settings needed for Traefik exposure.

#### Scenario: Keycloak profile enabled with invalid config
- **WHEN** Keycloak is enabled and required hostname/secrets/proxy settings are missing or invalid
- **THEN** preflight validation fails with a clear message before containers start

#### Scenario: Keycloak observability disabled
- **WHEN** Keycloak is enabled and observability option is disabled
- **THEN** Keycloak-specific observability checks do not block startup

#### Scenario: Keycloak observability enabled with unsafe metrics exposure
- **WHEN** Keycloak observability option is enabled and configuration would publicly expose metrics by default
- **THEN** preflight validation fails or warns with a clear remediation path according to the documented policy
