...
- [x] 1.1 Confirmar `traefik-quay` como proyecto soportado en el catálogo.
...
- [x] 1.4 Validar artefactos del cambio con `openspec validate add-project-traefik-quay --strict`.

## 2. Project Definition

- [x] 2.1 Crear estructura `deployment/projects/traefik-quay/`.
...
- [x] 2.3 Registrar `traefik-quay` en el catálogo para `deployment-project-list`.

## 3. Deployment Behavior

- [x] 3.1 Implementar tasks Ansible para sync de `compose-traeffik` en la VM objetivo.
- [x] 3.2 Ejecutar compose para los servicios del proyecto Quay definidos en manifiesto.
...
- [x] 3.5 Integrar autenticación de Quay con Keycloak (OIDC/SSO) según contrato del proyecto.
- [x] 3.6 Garantizar idempotencia razonable del flujo.
- [x] 3.7 Asegurar que Quay se publica detrás de Traefik y que la terminación TLS la gestiona Traefik según `tls_mode`.
...
- [x] 4.3 Documentar ejecución de `project=traefik-quay` y override TLS.
...
- [x] 5.1 Re-ejecutar `openspec validate add-project-traefik-quay --strict`.
