...
- [ ] 1.2 Validate change artifacts with `openspec validate add-qemu-rockylinux9-image-support --strict`.

## 2. QEMU Image Profile Definition

- [ ] 2.1 Add a `rockylinux9` OS/image profile for `target=qemu` in the provisioning interface.
- [ ] 2.2 Define pinned image source/version metadata for `Rocky Linux 9` (and checksum strategy if available).
...
- [ ] 4.1 Update `make help` / docs with the `rockylinux9` profile selection example for qemu (when implemented).
