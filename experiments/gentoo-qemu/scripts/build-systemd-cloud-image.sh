...
usage() {
  cat <<'EOF'
Build a Gentoo systemd qcow2 image suitable for libvirt + cloud-init (NoCloud).

This is an experimental builder used by the qemu/libvirt provisioning path for:
  make deployment os=gentoo init=systemd

The image is built from a Gentoo stage3 systemd tarball and uses:
  - Gentoo userland (systemd)
...
Usage:
  experiments/gentoo-qemu/scripts/build-systemd-cloud-image.sh [options]

Options:
  --output PATH           Output qcow2 path (default under infra/images/gentoo/systemd/)
  --work-dir PATH         Scratch directory (default: experiments/gentoo-qemu/work/build-systemd-cloud-image)
  --stage3-url URL        Gentoo stage3 systemd tarball URL
...
Environment overrides:
  GENTOO_SYSTEMD_STAGE3_URL
  GENTOO_SYSTEMD_STAGE3_SHA256
  GENTOO_SYSTEMD_IMAGE_SIZE_GB
  GENTOO_SYSTEMD_HOST_KERNEL_VERSION
  GENTOO_SYSTEMD_HOST_VMLINUZ
  GENTOO_SYSTEMD_HOST_INITRD
  GENTOO_SYSTEMD_HOST_MODULES_DIR
EOF
}
...
ROOTFS_MOUNT=""
LOOP_DEV=""
WORK_DIR_DEFAULT="${EXPERIMENT_ROOT}/work/build-systemd-cloud-image"
WORK_DIR="${WORK_DIR_DEFAULT}"
KEEP_WORK=false
FORCE=false
IMAGE_SIZE_GB="${GENTOO_SYSTEMD_IMAGE_SIZE_GB:-12}"
OUTPUT_QCOW2="${REPO_ROOT}/infra/images/gentoo/systemd/gentoo-systemd-cloudinit-hostkernel.qcow2"

STAGE3_URL_DEFAULT="https://distfiles.gentoo.org/releases/amd64/autobuilds/20260222T170100Z/stage3-amd64-systemd-20260222T170100Z.tar.xz"
STAGE3_URL="${GENTOO_SYSTEMD_STAGE3_URL:-${STAGE3_URL_DEFAULT}}"
STAGE3_SHA256="${GENTOO_SYSTEMD_STAGE3_SHA256:-}"

HOST_KERNEL_VERSION="${GENTOO_SYSTEMD_HOST_KERNEL_VERSION:-$(uname -r)}"
HOST_VMLINUZ="${GENTOO_SYSTEMD_HOST_VMLINUZ:-/boot/vmlinuz-${HOST_KERNEL_VERSION}}"
HOST_INITRD="${GENTOO_SYSTEMD_HOST_INITRD:-/boot/initrd.img-${HOST_KERNEL_VERSION}}"
HOST_MODULES_DIR="${GENTOO_SYSTEMD_HOST_MODULES_DIR:-/lib/modules/${HOST_KERNEL_VERSION}}"
...
RAW_IMG="${WORK_DIR}/gentoo-systemd.raw"
...
if [[ ! -f "${STAGE3_TAR}" ]]; then
  log "Downloading Gentoo stage3 systemd: ${STAGE3_URL}"
...
# Ensure first boot has networking even before cloud-init rewrites to static.
...
# systemd service wiring for cloud-init + sshd + networking.
...
log "Provisioning Gentoo packages and systemd services inside chroot (binpkg-only)"
...
menuentry 'Gentoo systemd (host-kernel bootstrap)' {
...
log "Gentoo systemd cloud-init image build complete"
